<template>
    <div class="withdrawCheck">
        <el-card class="box-card">
            <div slot="header" class="withDrawCheckTitle">
                <span>{{$t('home.withDrawCheck')}}</span>
                <el-button type="primary" @click="toReportExcel" :loading="exportLoading" style="float: right;margin-left: 5px;">{{$t('other.exportExcel')}}</el-button>
                ...
                <span class="checkKey">
                    <el-input v-model="keyWord" :placeholder="$t('placeholder.enterKeyword')" style="margin-right:5px;float: right;" @keyup.native.enter="reloadWithdrawList"></el-input>
                </span>
            </div>
            <div class="applyTable" v-loading="checkLoading">
                <el-table :data="tableData" ref="withdrawList" style="width: 100%" stripe border :height="userTableHeight">
                    <el-table-column v-for="(item,index) in tableHead" :key="index" :label="item.labelName" :prop="item.propName" :min-width="item.width" show-overflow-tooltip v-if="item.isShow">
                        <template slot-scope="scope">
                            ...
                            <span v-else-if="item.propName == 'LOGIN'">
                                <el-tooltip class="withdrawCountTip" effect="dark" :content="scope.row.WITHDRAW_COUNT == '0' ? $t('other.firstWithdraw') : $t('other.isWithdrawed') + scope.row.WITHDRAW_COUNT + $t('other.time')" placement="top-start">
                                    <span class="el-icon-my-iconcj" v-if="scope.row.WITHDRAW_COUNT == '0'"></span>
                                    <span class="countIcon" v-else>{{scope.row.WITHDRAW_COUNT}}</span>
                                </el-tooltip>
                                ...
                                <span class="holder-class" @click="showBalanceRecord(scope.row)">{{scope.row.LOGIN}}</span>
                            </span>
                    ...
                    <el-table-column class-name="operation-class" :label="$t('table.operate')" :min-width="250" v-if="!withdrawStatus" fixed="right">
                        <template slot-scope="scope">
                            <span @click="toEditWithdraw(scope.row)">
                                <el-button size="mini">{{$t('button.edit')}}</el-button>
                            </span>
                            <span @click="toSubmitWithdraw(scope.row)">
                                <el-button type="primary" size="mini" :loading="scope.row.affirmBtnLoading">{{$t('button.affirm')}}</el-button>
                            </span>
                            <span @click="toRejectWithdraw(scope.row)">
        ...
        <el-dialog :title="$t('dialog.editWithdraw')" :visible.sync="editVisible" width="80%" :before-close="handleClose" top="12vh">
            <div v-loading="editLoading">
                <div class="editWithdraw">
                    <el-row :gutter="15">
                        <el-form :model="editWithdrawForm" label-width="150px" ref="editWithdrawForm" :rules="editRules">
                            <el-col :span="12">
                                ...
                                <el-form-item :label="$t('home.currency')" prop="currency">
                                    <el-select v-model="editWithdrawForm.currency" clearable :placeholder="$t('home.selectCurrency')">
                                ...
                                <el-form-item :label="$t('home.amount')" prop="amount">
                                    <el-input v-model="editWithdrawForm.amount" :placeholder="$t('home.enterAmount')"></el-input>
                                </el-form-item>
                                <el-form-item :label="$t('home.charge')" prop="charge">
                                    <el-input v-model="editWithdrawForm.charge" :placeholder="$t('home.enterCharge')"></el-input>
                                </el-form-item>
                                ...
                                <el-form-item :label="$t('home.bankName')" prop="bank_name">
                                    <el-input v-model="editWithdrawForm.bank_name" :placeholder="$t('home.enterBankName')"></el-input>
                            ...
                            <el-col :span="12">
                                <el-form-item :label="$t('home.province')" prop="bank_province">
                                    <el-input v-model="editWithdrawForm.bank_province" :placeholder="$t('home.enterPorvince')"></el-input>
                                </el-form-item>
                                <el-form-item :label="$t('home.city')" prop="bank_city">
                                    <el-input v-model="editWithdrawForm.bank_city" :placeholder="$t('home.enterCity')"></el-input>
                                </el-form-item>
                                <el-form-item :label="$t('home.bankBranch')" prop="bank_branch">
                                    <el-input v-model="editWithdrawForm.bank_branch" :placeholder="$t('home.enterBankBranch')"></el-input>
                                </el-form-item>
                                <el-form-item :label="$t('home.bankAccount')" prop="card_number">
                                    <el-input v-model="editWithdrawForm.card_number" :placeholder="$t('home.enterBankAccount')"></el-input>
                                </el-form-item>
                                <el-form-item :label="$t('home.bankSwift')" prop="swift_code">
                                    <el-input v-model="editWithdrawForm.swift_code" :placeholder="$t('home.enterBankSwift')"></el-input>
                                </el-form-item>
                                <el-form-item :label="$t('home.bankIban')" prop="iban_code">
                                    <el-input v-model="editWithdrawForm.iban_code" :placeholder="$t('home.enterBankIban')"></el-input>
                                </el-form-item> 
                                <el-form-item :label="$t('home.sortCode')" prop="sort_code">
                                    <el-input v-model="editWithdrawForm.sort_code" :placeholder="$t('home.enterSortCode')"></el-input>
                                </el-form-item>
                            </el-col>
                        </el-form>
                    </el-row>
...
<script>
import { withdrawCheckList , withdrawSubmit, withdrawReject , editWithdraw , updateCharge } from "@/api/money/money"
import { getNewBalance , getBalanceRecord } from "@/api/user/user"
...
export default {
    name:'withdrawCheck',
      data() {
        return {
            tableData: [],
            tableHead: [
                { propName: 'LOGIN',labelName: this.$t('home.accountNumber'),width: '160',isShow: true },
                { propName: 'LOGIN_HOLDER',labelName: this.$t('home.holderName'),width: '110',isShow: true },
                { propName: 'FULL_NAME',labelName: this.$t('home.payeeName'),width: '100',isShow: true },
                { propName: 'USER_BALANCE',labelName: this.$t('home.balance'),width: '100',isShow: true },
                { propName: 'AMOUNT',labelName: this.$t('home.amount'),width: '100',isShow: true },
                { propName: 'CHARGE',labelName: this.$t('home.charge'),width: '160',isShow: true },
                { propName: 'BANK_PROVINCE',labelName: this.$t('home.province'),width: '100',isShow: true },
                { propName: 'BANK_CITY',labelName: this.$t('home.city'),width: '100',isShow: true },
                { propName: 'BANK_NAME',labelName: this.$t('home.bankName'),width: '120',isShow: true },
                { propName: 'BANK_BRANCH',labelName: this.$t('home.bankBranch'),width: '120',isShow: true },
                { propName: 'CARD_NUMBER',labelName: this.$t('home.bankAccount'),width: '120',isShow: true },
    ...
    methods: {
        /* 导出excel */
        toReportExcel(){
            //let tData = copyArr(this.tableData);
            let params = {
            ...
            withdrawCheckList(params).then( v => {
                ...
                if(v.code == 0){
                    ...
                    if(tData.length > 0){
                        ...
                        tDataKeys.push('WITHDRAW_COUNT');
                        tDataKeys.push('CARD_STATUS');
                        ...
                        tData.forEach( j => {
                            ...
                            /* 处理第几次出金 */
                            j.WITHDRAW_COUNT = j.WITHDRAW_COUNT == '0' ? this.$t('other.firstWithdraw') : this.$t('other.isWithdrawed') + j.WITHDRAW_COUNT + this.$t('other.time');
                            /* 是否验证 */
                            j.CARD_STATUS = j.CARD_STATUS == '1' ? this.$t('other.checked') : this.$t('other.noChecked');
                            /* 是否有负值 */
                            if(j.NEGATIVE_STATUS == '1'){
                                ...
                                j.NEGATIVE_STATUS = this.$t('other.plus');
                            }
                        })
                        exportExcelMethod(tData,tDataKeys,tDataKeys,'withdraw-check');
                    }else{
                        this.$message({
                            type: 'warning',
        ...
        /* 管理员 出金申请确认 */
        toSubmitWithdraw(row){

            this.$confirm(this.$t('other.confirmDeductions'), this.$t('home.hint'), {
        ...
        /* 修改出金申请 提交 */
        editSubmit(){
            this.$refs.editWithdrawForm.validate( (valid) => {
                if(valid){
                    let params = {
                        withdraw_id: this.editWithdrawForm.withdrawId,
                        login_holder: this.login_holder,
                        username: this.username,
                        currency: this.editWithdrawForm.currency,
                        amount: Number(this.editWithdrawForm.amount),
                        charge: Number(this.editWithdrawForm.charge),
                        full_name: this.full_name,
                        bank_name: this.editWithdrawForm.bank_name,
                        bank_province: this.editWithdrawForm.bank_province,
                        bank_city: this.editWithdrawForm.bank_city,
                        bank_branch: this.editWithdrawForm.bank_branch,
                        card_number: this.editWithdrawForm.card_number,
                        sort_code: this.editWithdrawForm.sort_code,
                        swift_code: this.editWithdrawForm.swift_code,
                        iban_code: this.editWithdrawForm.iban_code,
                    }
        ...
        showBalanceRecord(row){
            ...
            this.$nextTick( () => {
				let tableObj = this.$refs.recordTable.$el.getElementsByClassName('el-table__body-wrapper')[0];
        ...
        /* 修改手续费 */
        toEditCharge(row){
            this.$confirm(this.$t('other.sureEditCharge'), this.$t('home.hint'), {
                confirmButtonText: this.$t('button.confirm'),
                cancelButtonText: this.$t('button.cancle'),
                type: 'warning'
            }).then(() => {
                let params = {
                    withdraw_id: row.WITHDRAW_ID,
                    charge: row.chargeValue
                }
                updateCharge(params).then( v => {
                    if(v.code == '0'){
                        this.$message({
                            type: 'success',
                            message: this.$t('placeholder.editSuccess')
...
.negativeDetail .el-table tr,.negativeDetail .el-table th{
    background: black;
    color: #FFF;
    text-align: center;
}
.negativeDetail el-table--enable-row-hover,.negativeDetail .el-table__body tr:hover>td{
    background: #409EFF;
    cursor: pointer;
}
