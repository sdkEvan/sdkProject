<template>
	<div class="clientList" :class="{tableHeight: isExist,tabHeight: !isExist}" v-loading="loading">
		<el-table :data="tableData" style="width: 100%" @expand-change="changeExpand" stripe :row-class-name="setRowname" border ref="clientList" :height="userTableHeight">
			<el-table-column type="expand">
				<template slot-scope="props">
					<client-list v-if="props.row.LOGIN" :id="props.row.LOGIN" :pReportDate="pReportDate"></client-list>
				</template>
			</el-table-column>
		...
		<el-dialog title="" class="historyDialog" :visible.sync="dialogVisible" width="90%" :before-close="handleClose">
			...
			<div class="dialogContent" v-loading="tradeLoading">
				<el-table :data="tradeTable" ref="clientTradeTable" style="width: 100%" stripe border height="450">
...
import { getTradeList , getTradeHistory , getProUserinfo } from "@/api/user/user";
...
export default {
	...
	data() {
		return {
			...
			tableHead: [
				...
				{ propName: '',labelName: this.$t('table.countAcc'),width: '120' },
	...
	methods: {
        ...
        getData(id,page) {
			...
			if(this.pReportDate && this.pReportDate.length > 0){
				params.start_time_format = this.pReportDate[0];
				params.end_time_format = this.pReportDate[1];
            }
			this.loading = true;
		...
		scrollToLeft(){
			this.$nextTick( () => {
				let tableObj = this.$refs.clientTradeTable.$el.getElementsByClassName('el-table__body-wrapper')[0];
